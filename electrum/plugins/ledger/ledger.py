...
try:
    import ledger_runebase
    from ledgercomm.interfaces.hid_device import HID
    from ledger_runebase import WalletPolicy, MultisigWallet, AddressType, Chain
    from ledger_runebase.exception.errors import DenyError, NotSupportedError, SecurityStatusNotSatisfiedError
    from ledger_runebase.key import KeyOriginInfo
...
class Ledger_Client(HardwareClientBase):
    ...
    def construct_new(*args, device: Device, **kwargs) -> 'Ledger_Client':
        ...
        hid_device.open()
        transport = ledger_runebase.TransportClient('hid', hid=hid_device)
        try:
            cl = ledger_runebase.createClient(transport, chain=get_chain())
            cl.get_master_fingerprint()
        except (ledger_runebase.exception.errors.InsNotSupportedError,
                ledger_runebase.exception.errors.ClaNotSupportedError) as e:
            ...
            _logger.info(f"ledger_runebase.createClient() got exc: {e}. falling back to old plugin.")
            cl = None
        if isinstance(cl, ledger_runebase.client.NewClient):
...
class Ledger_Client_New(Ledger_Client):
    ...
    def __init__(self, hidDevice: 'HID', *, product_key: Tuple[int, int],
                 plugin: HW_PluginBase):
        Ledger_Client.__init__(self, plugin=plugin)
        transport = ledger_runebase.TransportClient('hid', hid=hidDevice)
        self.client = ledger_runebase.client.NewClient(transport, get_chain())
        self._product_key = product_key
        self._soft_device_id = None
    ...
    def sign_transaction(self, keystore: Hardware_KeyStore, tx: PartialTransaction, password: str):
        ...
        psbt = ledger_runebase.client.PSBT()
...
class LedgerPlugin(HW_PluginBase):
    ...
    def get_library_version(self):
        try:
            import ledger_runebase
            version = ledger_runebase.__version__
        except ImportError:
            raise
        except Exception:
            version = "unknown"
