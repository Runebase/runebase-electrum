...
class Huobi(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.huobi.pro', '/market/trade?symbol=runebase{}'.format(ccy.lower()))
...
class OKEX(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('www.okex.com', '/api/v1/ticker.do?symbol=runebase_{}'.format(ccy.lower()))
...
class Binance(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.binance.com', '/api/v3/avgPrice?symbol=RUNEBASE{}'.format(ccy.upper()))
...
class BitStamp(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        if ccy in CURRENCIES[self.name()]:
            json = await self.get_json('www.bitstamp.net', f'/api/v2/ticker/runebase{ccy.lower()}/')
...
class CoinCap(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coincap.io', '/v2/rates/runebase/')
    ...
    async def request_history(self, ccy):
        ...
        history = await self.get_json('api.coincap.io',
                                      '/v2/assets/runebase/history?interval=d1&limit=2000')
...
class CoinGecko(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coingecko.com', '/api/v3/coins/runebase/market_chart?vs_currency=%s&days=0' % ccy)
    ...
    async def request_history(self, ccy):
        history = await self.get_json('api.coingecko.com',
                                      '/api/v3/coins/runebase/market_chart?vs_currency=%s&days=max' % ccy)
