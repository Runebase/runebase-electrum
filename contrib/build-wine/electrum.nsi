...
;Variables
  ...
  !define PRODUCT_NAME "Runebase-Electrum"
  !define PRODUCT_WEB_SITE "https://github.com/runebase/runebase-electrum"
  !define PRODUCT_PUBLISHER "Runebase Team"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\Runebase-electrum-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\Runebase-electrum-${PRODUCT_VERSION}.exe" "" "$INSTDIR\Runebase-electrum-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\Runebase-electrum-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\Runebase-electrum-${PRODUCT_VERSION}.exe" 0


  ;Links runebase: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\runebase" "" "URL:runebase Protocol"
  WriteRegStr HKCU "Software\Classes\runebase" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\runebase" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\runebase\shell\open\command" "" "$\"$INSTDIR\Runebase-electrum-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\runebase"
